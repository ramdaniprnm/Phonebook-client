...
const PhonebookItem = (props) => {
  const { id, name, phone, avatar, updatePhonebook, throwDeleteModal, status } = props;
  ...
  const handleEditButton = async () => {
    try {
      ...
      console.error('Error entering update mode:', error);
    }
  };
          ...
          <form onSubmit={(e) => e.preventDefault()}>
            {isUpdate ? (
              ...
              <>
                ...
                {/* If this item is pending (unsent), show a rotating icon */}
                {status && status.operation === "add" && !status.sent && (
                  <FontAwesomeIcon icon={faRotate} spin title="Pending sync..." />
                )}
              </>
            )}
            <div className="button-group">
              <button
                type="submit"
                title="Update"
                onClick={isUpdate ? handleSaveClick : handleEditButton}
